...
class DualInputViT(nn.Module):
    ...
    def __init__(
        ...
        # initialize learnable stain embeddings
        self.he_embedding = nn.Parameter(torch.zeros(1, 1, self.embed_dim))
        self.ihc_embedding = nn.Parameter(torch.zeros(1, 1, self.embed_dim))
    ...
    def forward(
        ...
        ihc_x: torch.Tensor,
        ihc_pos: Optional[torch.Tensor],
        return_last_self_attention: bool = False,
        stain_embed: bool = True,
        pos_embed: bool = True,
    ) -> torch.Tensor:
        """ 
        ...
        Args:
            he_x:  Batch of input H&E images or embeddings as (batch, channels, height, width).
            he_pos:  Positions of H&E elements in the sequence as (batch, sequence, position).
            ihc_x:  Batch of input IHC images or embeddings as (batch, channels, height, width).
            ihc_pos:  Positions of IHC elements in the sequence as (batch, sequence, position).
        ...
        he_x = self.patch_embed_layer(he_x)
        ihc_x = self.patch_embed_layer(ihc_x)

        # convert to a sequence by combining the spatial dimensions if necessary
        # [B, C, H, W] -> [B, H*W, C] = [B, S, D]
        he_x = he_x.flatten(2, 3).transpose(1, 2)
        ihc_x = ihc_x.flatten(2, 3).transpose(1, 2)

        # add the learnable embeddings to the H&E and IHC embeddings
        if stain_embed:
            he_x += self.he_embedding
            ihc_x += self.ihc_embedding

        # concatenate the image embeddings and positions
        x = torch.cat([he_x, ihc_x], dim=1)
        pos = torch.cat([he_pos, ihc_pos], dim=1)
